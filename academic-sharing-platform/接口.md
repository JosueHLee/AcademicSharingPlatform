...
```json
{
  "code": 200, // 状态码：200=成功，400=参数错误，401=未登录/Token过期，403=权限不足，404=资源不存在，500=服务器错误
  "msg": "操作成功", // 提示信息
...
## 1.3 分页参数统一
请求参数固定为 `pageNum=1&pageSize=10`（pageNum 默认1，pageSize 默认10）
...
## 1.5 权限传递
所有接口（登录/注册除外）需在请求头携带 `Authorization: Bearer {Token}`

## 1.6 文件交互规则
非结构化文件（PDF/头像等）需传递华为云OBS返回的 `fileId`（文件唯一标识），不直接传递文件流


# 二、用户管理模块接口约束
## 2.1 科研人员注册（POST /api/users/researcher/register）
### 2.1.1 请求参数JSON
```json
{
  "username": "zhaomingze", // String，必填，用户名（手机号/邮箱）
  "password": "Aa123456!",  // String，必填，密码（8-20位+大小写+数字+特殊符号）
  "confirmPassword": "Aa123456!", // String，必填，与password一致
...
```json
{
  ...
  "data": {
    ...
    "certStatus": 0 // Number，认证状态（0=待审核，1=已通过，2=已拒绝）
  },
...
```json
{
  "username": "lisan", // String，必填
  "password": "Bb123456!", // String，必填
  "confirmPassword": "Bb123456!", // String，必填
  "email": "lisan@xxx.com" // String，必填
}
```
### 2.2.2 返回数据JSON
```json
...
```json
{
  ...
  "data": {
    ...
    "totalCount": 500, // Number，待同步数据总量
    "successCount": 0, // Number，已成功条数（实时更新）
    "failCount": 0 // Number，已失败条数
  },
  "timestamp": 1699999999999
}
```
...
## 6.3 数据同步状态查询（GET /api/sync/status/{syncLogId}）
### 6.3.1 请求参数JSON
```json
{} // 无请求参数（syncLogId在路径中传递）
```
### 6.3.2 返回数据JSON
```json
{
  ...
  "data": {
    "syncLogId": "sync001",
    "syncStatus": 2, // Number，1=进行中/2=完成/3=失败
    "progress": 100, // Number，同步进度（百分比）
    "successCount": 480,
    "failCount": 20,
    "failReason": "20条数据格式不兼容，已标记异常" // String，失败原因（可选）
  },
  "timestamp": 1699999999999
}
```

## 6.4 手动触发数据同步（管理员）（POST /api/admin/sync/manual）
### 6.4.1 请求参数JSON
```json
{
  "syncType": 0 // Number，必填，0=全量同步/1=公共数据库/2=学术搜索引擎
}
```
### 6.4.2 返回数据JSON
```json
{
  ...
  "msg": "同步任务已触发",
  "data": {
    "syncTaskId": "task001" // String，同步任务ID
  },
  "timestamp": 1699999999999
}
```
